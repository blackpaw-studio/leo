...
- Modify: `internal/cli/status.go` — **CRITICAL (carried from Task 2 review): the daemon route `GET /process/list` was deleted in Task 2, so `fetchProcessStates` (status.go:196) and `leo process.go:220` now 404 at runtime.** `process.go` is deleted wholesale here (removes that caller). For `leo status`: do NOT simply drop the live-state block (that permanently removes `leo status`'s live view). Instead **repoint it to the surviving agent endpoint `GET /api/agent/list`** — rename `fetchProcessStates`→`fetchAgentStates` hitting `/api/agent/list`, rename `report.Processes`/`report.ProcessStates`→`report.Agents`/`report.AgentStates`, and update the summary counts (142–146) + print line (296) to report live agent state (e.g. running/suspended counts) rather than process Enabled/Total. Verify the `/api/agent/list` response shape (see `handleAPIAgentList` in `internal/web`) and parse it accordingly. Net: `leo status` shows live AGENT state, no dangling `/process/list` call remains anywhere.
