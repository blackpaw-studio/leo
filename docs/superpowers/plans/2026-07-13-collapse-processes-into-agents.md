...
**Files:**
- Modify: `internal/web/web.go` — remove routes: `GET /processes` (238), `GET /processes/{name}` (239), `GET /partials/processes` (249), `POST /web/config/process/{name}` (264), `POST /web/process/add` (268), `DELETE /web/process/{name}` (269), `POST /web/process/{name}/interrupt|restart|send|message` (304–307). Remove `ProcessStateProvider` field/param only if it is exclusively process-facing; if it also feeds `/agents`, keep it and rename usages away from "process" as needed — verify before deleting.
...
- Modify: `internal/config/config.go`:
  ...
  - Remove the process branch in `Validate()` (586–641). Keep the template branch (643+) and the `DefaultStaleResumeHours` const (358) and `Defaults.StaleResumeHours` (186) — those remain valid defaults even though no scope reads them now; leave them (YAGNI to also rip out defaults). *(If lint flags `DefaultStaleResumeHours`/`ProcessStaleResume` removal as leaving `Defaults.StaleResumeHours` unused, keep the field — it is still validated at 546 and documented.)*
