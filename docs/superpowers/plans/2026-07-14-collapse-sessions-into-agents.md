...
- `internal/web/`: `handlers_sessions.go`, `/sessions` routes in `web.go`, `pages/sessions.html` + partials, layout nav entry + page dispatch; grep templates for `session` (primitive sense).
