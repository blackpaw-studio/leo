...
var socketDirs = []string{
	"/tmp/cc-socks",
	filepath.Join(os.Getenv("XDG_RUNTIME_DIR"), "cc-socks"),
...
func ResolveSocket(ctx context.Context, execFn ExecFunc, tmuxSession string) (string, error) {
	...
	panePID, err := execFn("tmux", tmux.Args("display-message", "-p", "-t", tmuxSession, "#{pane_pid}")...).Output()
	if err != nil {
	...
	for depth := 0; depth < 3; depth++ {
		...
		for _, pid := range frontier {
			...
			out, err := execFn("pgrep", "-P", pid).Output()
			if err != nil {
