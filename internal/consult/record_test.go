...
// timestamps are deterministic without sleeping.
...
func TestFileRecorderWritesRecordAndFramedStream(t *testing.T) {
	...
	if got := readRecord(t, dir, "c-0001"); got.Status != StatusQueued || got.Prompt != "what do you think?" {
...
func TestFileRecorderFlushesTrailingPartialLineOnClose(t *testing.T) {
	...
	if rec := readRecord(t, dir, "c-0003"); rec.Status != StatusFailed || rec.Error != "exit status 2" {
...
func TestFileRecorderSetStatusIsVisibleOnDisk(t *testing.T) {
	...
	if got := readRecord(t, dir, "c-0004").Status; got != StatusRunning {
		t.Fatalf("status = %q, want running", got)
	}
	if rec := readRecord(t, dir, "c-0004"); !rec.EndedAt.IsZero() {
...
func TestFileRecorderNeverPrunesUnfinishedConsults(t *testing.T) {
	r, dir := newTestRecorder(t)
	// The oldest consult is still running: pruning must skip it even once
	// the directory is over budget.
	oldest := testRecord("c-alive")
	oldest.StartedAt = oldest.StartedAt.Add(-time.Hour)
	live, err := r.Open(oldest)
	if err != nil {
		t.Fatalf("Open: %v", err)
	}
	...
	if _, err := os.Stat(filepath.Join(dir, "c-alive.json")); err != nil {
		t.Fatalf("running consult was pruned: %v", err)
	}
	// Still writable after the pruning passes.
	if _, err := live.Write([]byte("{\"type\":\"late\"}\n")); err != nil {
		t.Fatalf("Write after prune: %v", err)
	}
	if err := live.Close(StatusDone, nil); err != nil {
		t.Fatalf("Close: %v", err)
	}
	if events := readStream(t, dir, "c-alive"); len(events) != 1 {
		t.Errorf("got %d events, want the late write preserved", len(events))
	}
}
