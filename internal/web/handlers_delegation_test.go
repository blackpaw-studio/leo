...
func delegationServer(t *testing.T) (*Server, string) {
	...
	cfg := &config.Config{Templates: map[string]config.TemplateConfig{"one": {}, "two": {}}, Delegation: &config.DelegationConfig{
		ActiveProfile: "a",
		Profiles:      map[string]config.Profile{"a": {Roles: map[string]config.RoleTarget{"implement": {Template: "one", Model: "profile-model", Effort: "high"}}}, "b": {Roles: map[string]config.RoleTarget{"implement": {Template: "two"}}}},
	}}
...
func TestAPIDispatchRoleUsesFreshConfigAndExpectedTemplate(t *testing.T) {
	...
	recs := s.consults.Records()
	if len(recs) != 2 || recs[0].Template != "one" || recs[1].Template != "two" {
		t.Fatalf("records = %#v", recs)
	}
}
